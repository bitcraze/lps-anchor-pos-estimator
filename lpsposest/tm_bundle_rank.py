...
def tm_bundle_rank(sol, d):
    ...
    d2vm = dshuffle[np.ravel_multi_index(
    ...
    JJ = concatenate((JJ, ij2v[np.ravel_multi_index(
    ...
    for kkk in range(0, 5):
        ...
        aa = concatenate((np.linalg.norm(res), np.linalg.norm(
            res + jac * dz), np.linalg.norm(res2)), 1)
        bb = aa
        bb = bb - bb[1]
        bb = bb / bb[0]
