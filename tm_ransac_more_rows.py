	...
	for ii in tryrows:
		...
		for kk in range(1,sys.ransac_k2+1):
			...
			if len(tmp) >= 4:
				...
				xx = (d2n[0,trycols1]-zz[0,trycols1]) * linalg.inv(ZZ[:,trycols1])

				inlids = nonzero(abs(d2n[okcols] - (zz[okcols] + xx*ZZ[:,okcols])) < sys.ransac_threshold2)
		...
		if maxnrinl>sys.min_inliers2:
			...
			sol.inlmatrix[tmpsol.row, tmpsol.inlcols] = ones(1,len(tmpsol.inlcols))
